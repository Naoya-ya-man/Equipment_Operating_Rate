"""デモダッシュボード表示用データの整形(SQLiteバックエンド)。

`src.dashboard.data_service`と同じ構造だが、データ取得元が`src.demo.calculator`
(SQLite)である点のみ異なる。DataFrameの列構成はダッシュボード本体(`src.dashboard.charts`)
とそのまま互換性がある。
"""
...
from src.demo.calculator import (
...
def build_daily_trend(end_date: date, num_days: int, granularity: Granularity = "unit") -> pd.DataFrame:
    """直近num_days日分(end_date含む)の日別稼働率トレンドを構築する。"""
    records = []
...
def build_weekly_trend(end_date: date, num_weeks: int, granularity: Granularity = "unit") -> pd.DataFrame:
    """直近num_weeks週分(end_dateが属する週を含む)の週別稼働率トレンドを構築する。"""
...
def build_monthly_trend(end_date: date, num_months: int, granularity: Granularity = "unit") -> pd.DataFrame:
    """直近num_months ヶ月分(end_dateが属する月を含む)の月別稼働率トレンドを構築する。"""
    records = []
